...
EXPLORE_STEPS = 30 # How many exploration steps you'd like, should be larger than MEMORY_CAPACITY, feature exploration steps = EXPLORE_STEPS - MEMORY_CAPACITY
# ACTION_DIM = 2
# STATE_DIM = 4
...
class DQN(object):
    ...
    def learn(self):
        ...
        b_s_ = torch.FloatTensor(b_memory[:, self.STATE_DIM+self.ACTION_DIM + 1: -self.STATE_DIM-self.ACTION_DIM])
        b_a_ =  torch.LongTensor(b_memory[:, -self.ACTION_DIM:])

        net_input = torch.cat((b_s,b_a))
        q_eval = self.eval_net(net_input)
        net_input_ = torch.cat((b_s_,b_a_))
